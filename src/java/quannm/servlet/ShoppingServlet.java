...
//    private final String SHOPPING_PAGE = "shopping.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try { 
            ...
            dao.listProduct();
            List<tbl_productDTO> dto = dao.getListProductDTO();
            if (dto != null) {
                request.setAttribute("LIST_PRODUCT", dto);
                url = siteMaps.getProperty(ApplicationConstants.ShoppingFeature.SHOPPING_PAGE);
            }
        } catch (NamingException ex) {
            log("ShoppingServlet_ Naming: " + ex.getMessage());
        } catch (SQLException ex) {
            log("ShoppingServlet_ SQL: " + ex.getMessage());
        } finally {
